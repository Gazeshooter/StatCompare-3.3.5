...
function StatBuffs_GetBuffs(unit)
    ...
    while true do
        ...
        if not buffIconPath or i > 32 then
            break
        end
...
function StatCompare_AddBuffIconsToTooltip(tooltipFrame, unit)
    ...
    totalWidth = (outerPadding) + (iconSize + spacing) * iconCount - spacing
	local iconsPerRow = math.floor((tooltipFrame:GetWidth() - outerPadding) / (iconSize + spacing))
    ...
    for i, iconPath in ipairs(iconPaths) do
        ...
        tooltipFrame.buffTextures[i]:SetPoint("BOTTOMLEFT", tooltipFrame, "BOTTOMLEFT", outerPadding + (curIconInRow - 1) * (iconSize + spacing), (5 + (20 * (curRow - 1))))
    end
	tooltipFrame:Show()
end
...
function StatBuffs_UpdateBuffs(frameName, unit)
	local buffPanelPadding = 20 -- hack; not sure why the height is too short.
	...
	if buffFrame ~= nil then
		...
		if StatCompare_GetDisplayGroupSetting(bSelfStat, "ActiveBuffs") then
			...
			frame:SetHeight(frame:GetHeight() + buffFrame:GetHeight() + buffPanelPadding)
		else
